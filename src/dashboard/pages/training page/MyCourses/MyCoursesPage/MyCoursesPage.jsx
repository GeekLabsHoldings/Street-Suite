...
const MyCoursesPage = () => {
  ...
  async function getCompletedCourses() {
    ...
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_URL}courses/my_courses/completed/`,
        {
          headers: {
  ...
  async function getInProgCourses() {
    ...
    try {
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_URL}courses/my_courses/inprogress/`,
        {
          headers: {
            Authorization: `Token ${localStorage.getItem("userToken")}`,
          },
  ...
  return (
    <div className=" flex flex-col lg:flex-row gap-[50px] pt-[20px]">
      <div
        className={styles.trainings_courses + " myCourses-tabs w-full lg:w-2/3"}
      >
        <ul class="tabs-nav flex items-center gap-[20px] lg:gap-[40px] p-0 mt-0 mb-6 list-unstyled">
          ...
          <li className="pr-[20px] lg:pr-[40px]">
        ...
        <div class="tabs-stage tabs-content">
          ...
          <div id="inProgress" className="tab space-x-4">
            ...
            {inProg?.map((p, idx) => {
              return (
                <Link to={`/dashboard/training/my-courses/${p.course.slug}`}>
                  <div className={` ${styles.border_animate}`}>
                    <div className={styles.course_card + " flex gap-[16px]"}>
                      <div
                        className={
                          styles.course_card_img + " w-1/4 hidden lg:block "
                        }
                      ></div>
                      <div
                        className={
                          styles.course_card_info +
                          " w-full lg:w-3/4 space-y-2 lg:space-y-5"
                        }
                      >
                        ...
                        <div className="flex items-end justify-end lg:justify-between">
                          ...
                          <span>
                            Module {p.completed_modules} of{" "}
                            {p.course.number_of_modules}
                          </span>
                        </div>
                        <ProgressBar
                          completed={p.completed_modules / p.course.number_of_modules * 100}
          ...
          <div id="completed" className="tab space-x-4">
            {completed?.map((c, idx) => {
              return (
                <Link to={`/dashboard/training/my-courses/${c.course.slug}`}>
                  <div className={styles.border_animate}>
                    <div className={styles.course_card + " flex gap-[16px]"}>
                      <div
                        className={
